...
class VideoStorageManager {
    ...
    async saveVideo(videoData) {
        ...
        // Store as Blob (not Object URL) so it survives page refresh
        let thumbnail = videoData.thumbnail;
        if (!thumbnail && videoData.blob) {
            try {
                thumbnail = await this.generateThumbnail(videoData.blob);
            } catch (e) {
        ...
        try {
            ...
            // Browser storage quota hit: prune the oldest non-starred
            // videos and retry the write once before giving up.
            if (error && error.name === 'QuotaExceededError') {
                console.warn('[Storage] Quota exceeded, pruning old videos and retrying');
                await this.pruneOldVideos(StorageConfig.quotaPruneCount);
                await this.putVideo(video);
            } else {
                throw error;
            }
        ...
        // Keep the library bounded so long-term use can't fill the device.
        this.enforceQuota().catch((e) => {
            console.warn('[Storage] Quota enforcement failed:', e);
        });
    ...
    /**
     * Low-level put (single write attempt)
     */
    ...
    /**
     * Delete the N oldest non-starred videos
     */
    async pruneOldVideos(count) {
        const all = await this.getAllVideos(); // newest first
        const prunable = all.filter(v => !v.starred && v.quality !== 'starred').reverse(); // oldest first
        const toDelete = prunable.slice(0, count);
    ...
    /**
     * Enforce the max video cap (starred videos are never pruned)
     */
    async enforceQuota() {
        const total = await this.getVideoCount();
        if (total <= StorageConfig.maxVideos) return 0;
        return this.pruneOldVideos(total - StorageConfig.maxVideos);
    }
