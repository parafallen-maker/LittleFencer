...
| D20 | index 设置 | index.html 的设置模态框是死 UI：无入口按钮、四个开关无任何 JS 绑定 | 工具栏加 ⚙️ 入口，开关与 settings 双向绑定（实现单动作模式时一并接活） |
| D21 | UI 可用性 | 两页的 UI 事件绑定排在 MediaPipe 模型加载之后：模型加载慢/失败时设置面板完全失灵 | UI 绑定移至重型初始化之前 |
| D22 | 视频测试模式 | training-mode 设置的 `engine.skipFullBodyCheck` 在 engine 中从未被读取（空操作）：视频人物不满足全身可见时引擎永远卡在 WAITING，视频测试模式实际不可用 | engine.processPose 实现该开关（评测管线依赖此修复） |
